...
# distance threshold
close_threshold = 0.5  # units: meter
far_threshold = 1.5  # units: meter

def Target_Detection(image, depth_frame):
    ...
    for detection in output[0, 0, :, :]:
        ...
        if confidence > .4:
            ...
            depth_value = depth_frame.get_distance(int((box_x + box_width) / 2), int((box_y + box_height) / 2))
            # judge the kind of range
            if depth_value < close_threshold:
                obstacle_type = "Type 1: Close Obstacle"
            elif depth_value < far_threshold:
                obstacle_type = "Type 2: Far Obstacle"
            else:
                obstacle_type = "Type 3: No Obstacle"
            # draw a rectangle around each detected object
            cv.rectangle(image, (int(box_x), int(box_y)), (int(box_width), int(box_height)), color, thickness=2)
            # put the class name text on the detected object
            cv.putText(image, f"{class_name} {obstacle_type} {depth_value:.2f}m", (int(box_x), int(box_y - 5)), 
...
def depth_to_point_cloud(depth_image, intrinsics, depth_scale):
    ...
    depth_3d[:,:,0]=(depth_3d[:,:,0]+np.arange(w)-cx)*depth_3d[:,:,2]/fx
    depth_3d[:,:,1]=((depth_3d[:,:,1].T+np.arange(h)-cy)).T*depth_3d[:,:,2]/fy
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if state:
            color_frame = Target_Detection(color_image, aligned_depth_frame)
        ...
        # can add code visualization or save point_cloud 
