        ...
        for cluster in valid_clusters:
            ...
            if (closest_point[2] <= 2.0 or farthest_point[2] <= 2.0):#0.36right#-0.32left
                ...
                if center_position > 0:
                    judge_position = center_position - width/2
                else:
                    judge_position = center_position + width/2

                if (judge_position >= 0.58 or judge_position <= -0.92):
                    print("safe")
                elif(judge_position > 0.16 and judge_position < 0.58):
                    flag_right = 1
                elif(judge_position < -0.50 and judge_position > -0.92):#-0.92|-0.50|-0.17|0.16|0.58
                    flag_left = 1
                elif(judge_position >= -0.50 and judge_position <= 0.16):
                    flag_midian = 1
                center_points.append(closest_point)
                print("total",k,"NO.",center_position_count,":",closest_point,center_position)
    pin = flag_left*1 + flag_midian*2 +flag_right*4
    print(flag_left, flag_midian, flag_right)
